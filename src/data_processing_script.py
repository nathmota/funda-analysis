...
for provincie in provincies:
    ...
    def clean_price_m2(df):
        df['price_m2'] =  (df['price'] / df['size_m2']).astype(int)
        return df
    ...
    def get_script_element(url): 
        try:
            ...
            if response.status_code == 200:    #TODO else
            ...
            print("Exception not identified.")
            return np.nan
    ...
    df.to_csv(f'{raw_folder_path}/df_dropped_cols.csv', index=False)
    df2 = pd.read_csv(f'{raw_folder_path}/df_dropped_cols.csv', chunksize=chunk_size)

    existing_chunks = [int(chunk_file.split('_')[1].split('.')[0]) for chunk_file in os.listdir(chunk_folder)]
    if existing_chunks:
        chunk_index = max(existing_chunks) + 1
    else:
        chunk_index = 1


    for chunk_index, chunk in enumerate(df2, start=chunk_index):
        chunk = (
            chunk.pipe(clean_size)         
                .pipe(clean_price)
                .pipe(clean_price_m2)   
                .pipe(clean_year)       
                .pipe(clean_house_age)  
        ...
        print(provincie, chunk_index)
        file_path = os.path.join(provincie_folder, filename)
