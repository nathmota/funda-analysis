...
COLUMNS_TO_DROP = ['photo', 'descrip', 'log_id', 'insulation', 'ownership', 'building_type', 'last_ask_price_m2', 'date_list', 'listed_since', 'kind_of_house']
UNMISSABLE_COLUMNS = ['url', 'price', 'address','listed_since', 'zip_code', 'size', 'year', 'living_area', 'layout','city']
...
def clean_last_asked_price(df):
    df['last_asked_price'] = df['last_ask_price'].str.replace('.', '').str.extract(r'(\d+)').astype(int)
    return df
...
def clean_living_area(df):
    df['living_area'] = df['living_area'].str.extract(r'(\d+)').astype(int)
    df = df.rename(columns={'living_area': 'living_area_m2'})
    return df
...
def time_difference(text):
    ...
    if "year" in time_difference_string:
        time_unit = "year"   
    elif "month" in time_difference_string:
        time_unit = "month"         
    elif "week" in time_difference_string:
        time_unit = "week"
    elif "day" in time_difference_string:
        time_unit = "day"
    elif "hour" in time_difference_string:
        time_unit = "hour"
    elif "minute" in time_difference_string:
        time_unit = "minute"
    elif "second" in time_difference_string:
        time_unit = "second"        
    else:
...
def get_date_list(x):
    ...
    def period_formatting(period_text):
        period_formatting = {
            "weken": "week",
            "maanden": "month",
            "jaren": "year",
        }
        for dutch_period, english_period in period_formatting.items():
            if dutch_period in period_text:
                formatted_period = period_text.replace(dutch_period, english_period)
                return formatted_period
    if isinstance(x, str):
        match_date = re.search(r"'aangebodensinds' : '(.+?)'", x)  
        date = match_date.group(1)          
        pattern_date = r'\d+\s+\w+\s+\d+'
        pattern_period = r'weken|maanden|jaren'                      
...
for chunk in df:
    chunk = (
        chunk.pipe(clean_size)
            ...
            # .pipe(clean_last_asked_price)
            # .pipe(clean_year)       
            # .pipe(clean_house_age)  #transformation
            # .pipe(clean_living_area)
